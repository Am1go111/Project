...
def wifi(message):
  ...
  ws[f'I{str(k)}'] = otvet
  wb.save(fn)
  bot.send_message(message.chat.id,"Ответ: расположены в верном порядке.")
  time.sleep(10)
  bot.send_message(message.chat.id,"Дорогой, коллега! В нашей корпоративной культуре принято задавать вопросы своему руководителю, коллегам и HR команде. Надеюсь, рабочий день прошел продуктивно и интересно, на все вопросы нашлись ответы. Спасибо за прекрасный день в компании!")
  time.sleep(10)
  ...
  time.sleep(10)
  bot.send_message(message.chat.id, "Задание на сегодня: Ознакомиться с Правилами жизни офиса. Для этого необходимо зайти на «диск U-общие документы» и изучить раздел «Правила жизни офиса». Ниже в строке ответа, нужно указать к какому гостевому Wi-fi можно предложить гостю подключиться во нахождения в офисе CHG. ")
...
def day3(message):
  ...
  bot.send_message(message.chat.id,"Дорогой, коллега! В нашей корпоративной культуре принято задавать вопросы своему руководителю, коллегам и HR команде. Надеюсь, рабочий день прошел продуктивно и интересно, на все вопросы нашлись ответы. Спасибо за прекрасный день в компании!",reply_markup=markup)
  time.sleep(10)
  bot.send_message(message.chat.id,"Привет! Желаю максимально заряженного и вкуснейшего дня!")
  time.sleep(10)
  bot.send_message(message.chat.id,"Мы всегда стараемся расти и развиваться! Именно поэтому ориентируемся на лучшие мировые практики-стандарты LQA (Leading Quality Assurance). Это международная ассоциация Лидирующих отелей мира.  LQA необходимы, чтобы качественно проводить аудиты в отелях и улучшить качество обслуживания гостей. ")
  time.sleep(10)
  bot.send_message(message.chat.id,"Задание на сегодня: Оформить корпоративную подпись в почте. Перейти в папку U:\Общие документы - Правила жизни офиса, выбрать файл  «Правила жизни офиса_ПОДПИСЬ», ознакомиться с ним,  в Outlook создать свою корпоративную подпись.")
  time.sleep(10)
  bot.send_message(message.chat.id,"Дорогой, коллега! В нашей корпоративной культуре принято задавать вопросы своему руководителю, коллегам и HR команде. Надеюсь, рабочий день прошел продуктивно и интересно, на все вопросы нашлись ответы. Спасибо за прекрасный день в компании!")
  ...
  bot.send_message(message.chat.id,"COSMI на связи! Желаю прекрасного утра и мощного заряда позитивом")
  time.sleep(10)
  bot.send_message(message.chat.id,"Задание на сегодня: Ознакомиться с порядком оформления командировок. Для этого необходимо зайти на «диск U-общие документы- Командировки- инструкция по оформлению командировок». Чуть позже я отправлю квиз по этой теме")
  time.sleep(10)
  bot.send_message(message.chat.id,"Квиз: ")
  markup = types.ReplyKeyboardMarkup()
  markup.add(types.KeyboardButton("Самостоятельно через письмо-заявку в Космос Трэвел"))
  markup.add(types.KeyboardButton("Через руководителя "))
  markup.add(types.KeyboardButton("Команда HR забронирует все билеты и отели "))
  bot.send_message(message.chat.id, "1)Как осуществляется заказ билетов и бронирование гостиницы:", reply_markup=markup)
  bot.register_next_step_handler(message, odin)
def odin(message):
  otvet = message.text.strip()
  k = 1
  while message.chat.id != (ws[f'A{str(k)}'].value):
  ...
  ws[f'P{str(k)}'] = otvet
  wb.save(fn)
  bot.send_message(message.chat.id, "Верный ответ: Самостоятельно через письмо-заявку в Космос Трэвел")
  markup = types.ReplyKeyboardMarkup()
  markup.add(types.KeyboardButton("В течение 3-х дней "))
  markup.add(types.KeyboardButton("В день завершения командировки "))
  markup.add(types.KeyboardButton("В течение рабочей недели  "))
  bot.send_message(message.chat.id, "2) В течение какого времени необходимо заполнить и отправить отчет о выполнении задания", reply_markup=markup)
  bot.register_next_step_handler(message, dva)
def dva(message):
  otvet = message.text.strip()
  k = 1
  while message.chat.id != (ws[f'A{str(k)}'].value):
  ...
  ws[f'Q{str(k)}'] = otvet
  wb.save(fn)
  bot.send_message(message.chat.id, "Верный ответ: В течение 3-х днейл")
  markup = types.ReplyKeyboardMarkup()
  markup.add(types.KeyboardButton("Отправить в бухгалтерию"))
  markup.add(types.KeyboardButton("Передать своему руководителю"))
  markup.add(types.KeyboardButton("Передать в HR отдел "))
  bot.send_message(message.chat.id, "3)Кому необходимо передать все документы и документы, подтверждающие расходы? ", reply_markup=markup)
  bot.register_next_step_handler(message, tri)
def tri(message):
  otvet = message.text.strip()
  k = 1
  while message.chat.id != (ws[f'A{str(k)}'].value):
  ...
  ws[f'R{str(k)}'] = otvet
  wb.save(fn)
  bot.send_message(message.chat.id, "Верный ответ: Отправить в бухгалтерию")
  markup = types.ReplyKeyboardMarkup()
  btn1 = types.KeyboardButton("Не работает принтер")
  btn2 = types.KeyboardButton("Не работает Wi-fi")
  markup.row(btn1, btn2)
