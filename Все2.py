...
def helpbot(message):
  bot.forward_message(6755650947, message.chat.id, message.message_id)
  bot.register_next_step_handler(message, handle_text)

@bot.message_handler(content_types=["text"])
def handle_text(message):
  if int(message.chat.id) == int(6755650947):
...
def helpBot(m):
     bot.forward_message(6755650947, m.chat.id, m.message_id)
